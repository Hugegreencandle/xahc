...
`/execute`) · `prove` (→ xahc-prover; `--invariant limit|guardrail|termination|monotonic|nospend|conservation|limit-iou|authz|validate|overflow|foreign-authz|reserve|time-nonce|emission`) · `doctor` · `new`.
